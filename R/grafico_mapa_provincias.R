...
#' Los datos se relacionan con la cartografía mediante el código provincial.
#'
...
#'   código provincial. Por defecto, `"cod_provincia"`.
...
#'
#' @details
#' Cada provincia debe tener como máximo una observación. La función no agrega
...
#' @import sf
#' @export
grafico_mapa_provincias <- function(
    datos,
  ...
  # ---------------------------------------------------------------------------
  # Preparación de códigos provinciales
  # ---------------------------------------------------------------------------
  ...
  if (anyNA(codigos_numericos)) {
    stop(
      paste0(
        "La columna `",
        codigo,
        "` contiene códigos provinciales no válidos."
      ),
      call. = FALSE
    )
  }

  datos_plot$.cpro_grafico <- sprintf(
    "%02d",
    codigos_numericos
  )

  if (anyDuplicated(datos_plot$.cpro_grafico)) {
    stop(
      paste0(
        "Existen varias observaciones para una misma provincia. ",
  ...
  codigos_validos <- as.character(mapa_provincias$cpro)

  codigos_desconocidos <- setdiff(
    datos_plot$.cpro_grafico,
    codigos_validos
  )
  ...
  if (length(codigos_desconocidos) > 0L) {
    stop(
      paste0(
        "Códigos provinciales no reconocidos: ",
  ...
  indice <- match(
    as.character(mapa_provincias$cpro),
    datos_plot$.cpro_grafico
  )

  mapa_plot <- mapa_provincias

  mapa_plot$.valor_grafico <- datos_plot[[valor]][indice]

  # Ceuta y Melilla son también códigos provinciales 51 y 52.
  if (!incluir_ceuta_melilla) {
    mapa_plot <- mapa_plot[
      !as.character(mapa_plot$cpro) %in% c("51", "52"),
      ,
      drop = FALSE
    ]
  }
